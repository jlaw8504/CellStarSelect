function dimCell = spotHeight(dataCell, regionSize, spbChannel)
...
num = size(dataCell,1);
dimCell = cell([num, 4]);
dimCell(1,:) = {'Kinet1', 'Kinet2', 'SPB1', 'SPB2'};
%% Set up waitbar
...
for n = 2:size(dataCell,1)
    ...
    [dimCell{n,1}(2), dimCell{n,1}(1), dimCell{n,1}(3)] = calcFwhm(rotKinet1);
    [dimCell{n,2}(2), dimCell{n,2}(1), dimCell{n,2}(3)] = calcFwhm(rotKinet2);
    [dimCell{n,3}(2), dimCell{n,3}(1), dimCell{n,3}(3)] = calcFwhm(rotSpb1);
    [dimCell{n,4}(2), dimCell{n,4}(1), dimCell{n,4}(3)] = calcFwhm(rotSpb2);
    %% Update waitbar
    waitbar(n/num);
end
%% Close waitbar
