...
for n = 2:size(dataCell,1)
    ...
    try
        [finalspbimage,newspb1,newspb2,finalkinetimage,newkinet1,newkinet2] = rotatethis(spbIm,spb1,spb2,kinetIm);
        spbSub = spb1 - spb2;
        kkSub = kinet1-kinet2;
        ...
        rotKinet1 = extractFoci(newkinet1, finalkinetimage, regionSize);
        rotKinet2 = extractFoci(newkinet2, finalkinetimage, regionSize);
        rotSpb1 = extractFoci(newspb1, finalspbimage, regionSize);
        rotSpb2 = extractFoci(newspb2, finalspbimage, regionSize);
        ...
        %% Calculate the FWHM of the rotated images
        heightArray.kHeights1(n-1) = nan;
        heightArray.kHeightsRsq1(n-1)= nan;
        heightArray.kHeights2(n-1) = nan;
        heightArray.kHeightsRsq2(n-1) = nan;
        heightArray.sHeights1(n-1) = nan;
        heightArray.sHeightsRsq1(n-1) = nan;
        heightArray.sHeights2(n-1) = nan;
        heightArray.sHeightsRsq2(n-1) = nan;
        %% Calculate the 2D Spindle Length in Pixels
        heightArray.sLengths(n-1) = norm(spbSub(1:2));
        heightArray.kkDistance(n-1)= norm(kkSub(1:2));
        %% Update waitbar
