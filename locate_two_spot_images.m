function [img1Spots, img2Spots, xyList, polygonList] = locate_two_spot_images(polygon_data, im1, im2)
...
%           spot while columns are Y (row) and X (column).
 ...
 polygonList = {};
 cnt = 1;
for n = 1:numel(polygon_data.in_polygon_list)
    inPolygon = polygon_data.in_polygon_list{n} == n;
    if size(inPolygon, 1) < 5 || size(inPolygon, 2) < 5
        continue
    end
    cc = bwconncomp(inPolygon);
    if cc.NumObjects ~= 1
        continue
    end
    inPolygonXY = double(polygon_data.polygon_xy_list(n,:) + 1);
